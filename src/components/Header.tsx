...
import { setCurDate } from '@/redux/reducers/calendarReducer';
...
function Header() {
  ...
  const onClickToday = () => {
    dispatch(setCurDate(formatISO(new Date(), { representation: 'date' })));
  };

  const onClickPrev = () => {
    dispatch(
      setCurDate(
        formatISO(addWeeks(selectedDate, -1), { representation: 'date' })
      )
    );
  };
  ...
  return (
    <header className="flex flex-row basis-16 p-2 outline outline-1 outline-gray-300">
      <div className="flex flex-row items-center shrink w-[238px] h-full pr-[30px]">
        <svg
          className="fill-gray-600 w-12 h-12 p-3 mx-1"
          focusable="false"
          viewBox="0 0 24 24"
        >
      ...
      <div className="flex flex-row grow h-full items-center">
        <button
          className="h-[34px] px-[12px] py-[7px] ml-[8px] mr-[12px] py-bg-transparent outline outline-1 outline-gray-300 headerHoverColor text-sm rounded"
          onClick={onClickToday}
        >
          오늘
        </button>
        <button
          className="h-8 w-8 ml-2 bg-transparent text-sm rounded-full headerColor headerHoverColor"
          onClick={onClickPrev}
        >
        ...
        <button
          className="h-8 w-8 bg-transparent text-sm rounded-full headerColor headerHoverColor"
          onClick={onClickNext}
        >
        ...
        <p className="ml-[10px] p-[8px] text-[22px] text-[#3C4043]">
          {selectedWeek && calcMonth(selectedWeek)}
        </p>
      </div>
      <div className="flex flex-row h-full items-center">
        <button className="h-10 w-10 bg-transparent text-sm rounded-full headerColor headerHoverColor">
          <span className="material-symbols-sharp pt-1.5">search</span>
        </button>
        <button className="h-10 w-10 bg-transparent text-sm rounded-full headerColor headerHoverColor">
          <span className="material-symbols-sharp pt-1.5">help</span>
        </button>
        <button className="h-10 w-10 bg-transparent text-sm rounded-full headerColor headerHoverColor">
          <span className="material-symbols-sharp pt-1.5">settings</span>
        </button>
        <DropDown />
      </div>
      <div className="flex flex-row w-[120px] h-full items-center pl-[30px] pr-[4px]">
        <button className="h-10 w-10 bg-transparent text-sm rounded-full headerColor headerHoverColor">
          <span className="material-symbols-sharp pt-1.5">apps</span>
        </button>
        <img
          className="h-10 w-10 bg-transparent text-sm rounded-full headerColor headerHoverColor p-1"
